...
# this taked from python2.7 urllib.urlencode to meet encoding.
def urlencode(query, doseq=0, encoding='utf-8'):
    ...
    The encoding specifies the encoding to encode unicode values (default is 'utf-8').
    """

    def smart_str(value):
        if isinstance(value, unicode) or hasattr(value, '__unicode__'):
    ...
    if not doseq:
        ...
        for k, v in query:
            k = quote_plus(smart_str(k))
            v = quote_plus(smart_str(v))
            l.append(k + '=' + v)
    else:
        for k, v in query:
            k = quote_plus(smart_str(k))
